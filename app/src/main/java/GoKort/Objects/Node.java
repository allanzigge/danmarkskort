...
public class Node implements Serializable {
    ...
    // Used the "afstandformel" to determine the distance between two points in a 2
    // dimensional space
    public double distToVertex(Node other) {
        double dist = (Math.pow(Math.abs((lat - (other.lat))), 2.0)
                + Math.pow(Math.abs(((lon - other.lon) * 0.56)), 2.0));
