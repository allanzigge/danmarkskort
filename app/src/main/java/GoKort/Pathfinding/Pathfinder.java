...
import GoKort.DataStructures.MinPQ;
import GoKort.DataStructures.RTree;
...
public class Pathfinder implements Serializable {
        ...
        while (open.size() > 0 && System.currentTimeMillis() - 5000 < timer) {
            ...
            if (next.originalNode.getID() == to.getID()) { // If this is the goal:
                ...
                for (Edge edge : next.originalNode.neigbors) { // Explore each nieghbor(option) from the current node
                    if (edge.isDrivable) {
                        if ((!closed.contains(vertexMap.get(edge.toID)))) { // Only necessary to explore un-explored
                                                                            // nodes
                            PathNode nextNode;
                            if (map.keySet().contains(vertexMap.get(edge.toID))) {
                                nextNode = map.get(vertexMap.get(edge.toID)); // If we know a way to this node, get the
                                                                              // node
                            } else {
                            ...
                            double newCost = next.costToNode + (edge.cost / edge.road.getSpeed()); // Notice the speed
                                                                                                   // limit affects the
                                                                                                   // cost
                            map.put(vertexMap.get(edge.toID), nextNode); // Save this node

                            if (newCost < nextNode.costToNode) { // IF the calculated cost is cheaper than the already
                                                                 // known path
    ...
    private void createTextRoute(ArrayList<Edge> edges) {
        ...
        if (3600 < timeEstimate) { // Formating the text description
            setTravelTime(Float.toString((float) Math.floor(timeEstimate / 3600)) + " t "
                    + Float.toString((float) Math.ceil((timeEstimate % 3600) / 60)) + " m");
        } else {
        ...
        setTravelLength("Længde: " + totalLength + " km"); // Tweaked value a bit to make length estimate a bit more
                                                           // precise
        travelType = "default";
    }
