...
public class Controller {
    ...
    public Controller(Model model, View view) {
        ...
        view.canvas.setOnMouseMoved(e -> {
            ...
            if((view.scalebar.getScale()*view.canvasHeighScale*view.canvasWidthScale)<500) {
                ...
                view.nearestRoad.setText(((Highway) model.smallRoadRtree.NNSearch(new float[]{view.position.latPosition,view.position.lonPosition})).toString());
                }
            } else if((view.scalebar.getScale()*view.canvasHeighScale*view.canvasWidthScale)<5000) {
                if(!model.mediumRoadRTree.isEmpty) {
                view.nearestRoad.setText(((Highway) model.mediumRoadRTree.NNSearch(new float[]{view.position.latPosition,view.position.lonPosition})).toString());
                }
            } else {
                if(!model.bigRoadRTree.isEmpty) {
                view.nearestRoad.setText(((Highway) model.bigRoadRTree.NNSearch(new float[]{view.position.latPosition,view.position.lonPosition})).toString());
                }
            }
        ...
        view.scene.heightProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                ...
                view.findRouteFromTextField.textProperty().addListener((observable, oldValue, newValue) -> {
                    ...
                    setAdressOptionBox(view.findRouteFromTextField, view.searchResultFromVBox, "searchFromNode" ,newValue, model, view);
                ...
                view.findRouteToTextField.textProperty().addListener((observable, oldValue, newValue) -> {
                    ...
                    setAdressOptionBox(view.findRouteToTextField, view.searchResultToVBox, "searchToNode",newValue, model, view);
            ...
            view.favoritesButton.setOnMouseClicked(e-> {
                    if(view.favoritesButton.getBackground().getFills().get(0).getFill().equals(Color.WHITE) && view.searchFromNode!=null ) {
                    ...
                    Button button = view.layout.copyButton(view.searchMenu, view.layout.copyImageView(view.serachLoopImage));
            ...
            view.favoritesButton2.setOnMouseClicked(e-> {
                if(view.favoritesButton2.getBackground().getFills().get(0).getFill().equals(Color.WHITE) && view.searchFromNode!=null && view.searchToNode!=null ) {
                    ...
                    Button button = view.layout.copyButton(view.findRouteMenu, view.layout.copyImageView(view.findRouteImage)); 
            ...
            view.clearFindRouteButton.setOnAction(e-> {
                ...
                view.findRouteFromTextField.clear();
                view.findRouteToTextField.clear();
                view.favoritesButton2.setBackground(new Background(new BackgroundFill(Color.WHITE,CornerRadii.EMPTY, Insets.EMPTY)));
                view.searchFromNode = null;
                view.searchToNode = null;
                model.route.clear();
                view.redraw();
	...
	private Label createDropdownOptions(Address adr, TextField textField, String searchNode, View view, Model model) {
		...
		private void setOnClickEventSerachResult(String searchNode, TextField textField, Address adr, Label label, View view, Model model) {
		...
		private void findRoute(Model model, View view) {
            if(transportType.equals("car")) {
                Edge fromEdge = (Edge) model.edgeTreeCar.NNSearch(new float[]{view.searchFromNode.lat,view.searchFromNode.lon});
                Edge toEdge = (Edge) model.edgeTreeCar.NNSearch(new float[]{view.searchToNode.lat,view.searchToNode.lon});
                model.route = model.pathfinder.findPathCar(model.vertexMap.get(fromEdge.toID),model.vertexMap.get(toEdge.toID));
            } else if(transportType.equals("bike") | transportType.equals("walk")){
                Edge fromEdge = (Edge) model.edgeTreeBike.NNSearch(new float[]{view.searchFromNode.lat,view.searchFromNode.lon});
                Edge toEdge = (Edge) model.edgeTreeBike.NNSearch(new float[]{view.searchToNode.lat,view.searchToNode.lon});
                model.route = model.pathfinder.findPathBike(model.vertexMap.get(fromEdge.toID),model.vertexMap.get(toEdge.toID));
            }
        }
        ...
        private void menuSelected(Button button, StackPane stackPane, View view) {
            if (button.getBackground().getFills().get(0).getFill().equals(Color.LIGHTGREY)) {
                ...
                view.favoritesButton.setBackground(new Background(new BackgroundFill(Color.WHITE,CornerRadii.EMPTY, Insets.EMPTY)));
